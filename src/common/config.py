...
def show_config(config):
    ...
    log('info', f"Communication channel : {'TCP' if config.mp else 'Shared memory'}")
    log('', f"Verbose: {config.verbose}")
    log('success', f'Network configuration:')
    log('info', f"Total number of nodes: {config.num_nodes}")
    ...
    log('info', f"Number of announcers to subscribe to: {config.Qsub}")
